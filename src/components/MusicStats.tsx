...
import { AudioLines, UserRound, ArrowLeft, ArrowRight, Disc3 } from 'lucide-react';
            ...
            <Card className="rounded-lg border border-zinc-800 relative overflow-hidden bg-zinc-900/90">
              <CardHeader className="relative z-10 flex justify-between items-center py-5 px-6">
                ...
                <div className="flex flex-col items-center mx-auto w-full">
                  ...
                  <span className="text-xs text-zinc-500 mt-2 px-2 rounded-md bg-zinc-800/35">since 4 weeks</span>
                </div>
              </CardHeader>
                      ...
                      >
                        {activeTab === 'artists' ? (
                          getCurrentItems().map((item) => {
                            ...
                            return (
                              <div key={item.position} className="flex items-center gap-3 group">
                                ...
                                >
                                  <div className="relative w-12 h-12">
                                    {loadingImages[item.position] !== false && (
                                    ...
                                    <Image
                                      ...
                                      onLoad={() => handleImageLoad(item.position)}
                                    />
                                  </div>
                                  <div className="flex-1">
                                    <h3 className="font-semibold group-hover:text-[#c6c6c6] transition-colors">{item.artist.name}</h3>
                            ...
                            return (
                              <div key={item.position} className="flex items-center gap-3 group">
                                ...
                                >
                                  ...
                                  <div className="flex-1">
                                    <h3 className="font-semibold group-hover:text-[#c6c6c6] transition-colors">{item.track.name}</h3>
                                    <p className="text-sm text-gray-500">
                                      {Array.from(new Set(item.track.artists.map(artist => artist.name)))
                                        .reduce((acc, name, index, arr) => {
                                          if (index === 0) return name;
                                          if (index === arr.length - 1) return `${acc} & ${name}`;
                                          return `${acc}, ${name}`;
                                        }, '')}
                                    </p>
                                  </div>
