...
export async function GET() {
  try {
    ...
    const reposRes = await fetch('https://api.github.com/users/refurbishing/repos', {
      headers: {
    ...
    const filteredRepos = repos.filter((repo) => 
      !repo.fork || repo.full_name === 'refurbishing/dots-hyprland'
    );

    const languagePromises = filteredRepos.map((repo) => 
      fetch(repo.languages_url, { next: { revalidate: 18000 } }).then(res => res.json())
    );
